# -*- coding: utf-8 -*-
"""
Created on Tue Mar  3 01:24:47 2020
...
class MusicItem():
    ...
    @classmethod
    def from_file(cls, midi_file, vocab, encode): 
        #encode = MusicEncode(vocab)
        return cls.from_stream(encode.file_stream(midi_file), vocab, encode)
    @classmethod
    def from_stream(cls, stream, vocab, encode):
        if not isinstance(stream, music21.stream.Score): stream = stream.voicesToParts()
        chordarr = encode.stream_chordarr(stream) # 2.
        npenc = encode.chordarr_npenc(chordarr) # 3.
        return cls.from_npenc(npenc, vocab, encode, stream)
    @classmethod
    def from_npenc(cls, npenc, vocab, encode, stream=None): 
        enc = encode.npenc2idxenc(npenc, vocab)
        return MusicItem(enc, vocab, stream)
